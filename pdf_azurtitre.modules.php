 ...
 *	\ingroup    propale
 *	\brief      Fichier de la classe permettant de generer les propales au modele Azur avec des lignes de titre
 ...
 */
class pdf_azurtitre extends ModelePDFPropales
{
	/**
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					// VAT Rate
					if (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT') && !getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN') && empty($isTitre) && empty($isComment)) {
					...
					// Unit
					if(getDolGlobalInt('PRODUCT_USE_UNITS') && empty($isTitre) && empty($isComment)) {
